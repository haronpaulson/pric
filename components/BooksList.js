...
function BooksList() {
  ...
  return (
    <div className=' sm: space-x-6 md:space-x-12  '>
      <div>
      ...
      {movies.map(function(movie) {
        ...
        <img src = {movie.picture} alt="boooks" className='w-28  object-contain md:w-48'/>
        <Image src={prefix + movie.picture} alt="bookss" width="170" height="70" objectFit='contain' className='hidden'/>
        <div>
            <p1 > {movie.name}</p1><br/>
            <p6 className="mb-"><span className='text-sm underline'>{movie.date}</span></p6><br/>
            <a href= {movie.webpage} target= "_blank">
            <button type='text' className=' mt-10 ml-3 border-2 p-1 w-20 rounded-full font-semibold border-white text-black bg-white butt'>Buy Now</button>
            </a>
        </div>
